...
- [x] Explain the differences between `client-side routing` and `server-side routing`.
      Client side routing changes the URL but does not reload the page, only re-rendering the sections that need to be changed. It usually
      takes longer to load because you're loading everything up front which makes for a faster experience when perusing the app.
      Server-side routing renders a new page and serves the page to the client browser needing a page refresh. It's also a little
      faster then client-side routing because you're only loading what you need.
